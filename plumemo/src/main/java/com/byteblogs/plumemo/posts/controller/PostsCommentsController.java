...
public class PostsCommentsController {
    ...
    @LoginRequired(role = RoleEnum.USER)
    @PostMapping("/comments/v1/addReply")
    public Result saveMyPostsComments(@RequestBody MyPostsComments myPostsComments) {
        return this.postsCommentsService.saveMyPostsComments(myPostsComments);
    }
    ...
    @PostMapping("/comments/list/{current}/{pageSize}")
    public Result<MyPostsComments> getCommentsList(@PathVariable("current") Integer current, @PathVariable("pageSize") Integer pageSize
    , @RequestParam("postsId") Long postsId, @RequestParam("parentId") Long parentId){
        return postsCommentsService.getCommentsList(current,pageSize,postsId,parentId);
    }

    @PostMapping("/comments/getReplyList")
    public Result<MyPostsComments> getReplyList(@RequestParam("postsId") Long postsId,
                                                @RequestParam("parentId") Long parentId,@RequestParam("replyCount") Integer replyCount){
        return postsCommentsService.getReplyList(postsId,parentId,replyCount);
    }
}
